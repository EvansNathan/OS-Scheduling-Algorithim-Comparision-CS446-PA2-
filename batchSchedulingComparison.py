...
# FCFS - First Come, First Served takes processes and executes them in the order in which they are received
# It does not take into account priority or the execution time of the processes. This can cause an extremely
# long process to back up and delay other processes.

# Shortest First - Always prioritizes the shortest job first over. This will not take priority or arrival time
# into account. This can cause longer processes to not execute iof shorter processes continue to interrupt.

# Priority - With the non-preemptive priority algorithm the OS can assign important processes to be executed
# before other processes. This will not interrupt the current process running so once started each process will
# go to completion.
